...
const SpecularButton = ({
  ...
  const hasFill = className.includes("specular-button--primary");
  const isWide = className.includes("specular-button--wide");
  const classes = `specular-button specular-button--${size}${className ? ` ${className}` : ""}`;

  // FX sits outside the button so overflow:hidden on primary can clip the
  // yellow fill without chopping the specular canvas (inset -20px).
  const inner = (
    <>
      {hasFill ? (
        <span className="specular-button__fill" aria-hidden="true" />
      ) : null}
  ...
  return (
    <span
      className={`specular-button-wrap${isWide ? " specular-button-wrap--wide" : ""}`}
