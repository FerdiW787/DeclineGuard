...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    float tone=g(LF+w);
    // Bright core leans toward uColor (yellow); soft fog/edges lean toward uColor2 (purple)
    float tintMix=clamp(tone*2.2,0.0,1.0);
    vec3 tint=mix(uColor2,uColor,tintMix);
    vec3 beam=tone*tint;
    ...
    beam*=eM;
    beam*=uFade;
    fc=vec4(uBgColor + beam + dith, 1.0);
}
...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    const targetFrameMs = fpsCap > 0 ? 1000 / fpsCap : 0;
    let lastRenderMs = 0;
    ...
    const animate = () => {
      ...
      // Frame-rate cap: skip this rAF tick if not enough time has passed.
      if (targetFrameMs > 0) {
        const nowMs = performance.now();
        if (nowMs - lastRenderMs < targetFrameMs - 1) return;
        lastRenderMs = nowMs;
      }
      ...
      if (targetFrameMs <= 0) fpsSamplesRef.current.push(instFps);
      ...
      // Adaptive DPR only runs when uncapped, to avoid periodic resize hitches.
      if (targetFrameMs <= 0) adjustDprIfNeeded(performance.now());
    };
  ...
  useEffect(() => {
    ...
    const secondary = hexToRGB(color2 || color || '#FFFFFF');
    uniforms.uColor2.value.set(secondary.r, secondary.g, secondary.b);

    const bg = hexToRGB(backgroundColor || '#000000');
    uniforms.uBgColor.value.set(bg.r, bg.g, bg.b);
  }, [
    wispDensity,
    mouseTiltStrength,
