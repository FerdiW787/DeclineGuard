...
export interface ChartTooltipProps {
  ...
  /**
   * Override tooltip dot fill. When omitted, dots use each series stroke,
   * or a matching `rows` entry via `dataKey`.
...
const ChartTooltipInner = memo(function ChartTooltipInner({
  ...
  const resolveDotColor = useMemo(() => {
    return (line: LineConfig, index: number): string => {
      ...
      if (rowsRenderer && tooltipRows.length > 0) {
        const byKey = tooltipRows.find((row) => row.dataKey === line.dataKey);
        if (byKey?.color) {
          return byKey.color;
        }
        // Only fall back to index when every row is keyed the same as lines
        // (legacy: rows order === series order). Prefer series stroke otherwise.
        const allKeyed = tooltipRows.every((row) => row.dataKey);
        if (!allKeyed && tooltipRows[index]?.color) {
